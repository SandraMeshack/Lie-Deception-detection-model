...
size = 8    #number of images to train at a time
...
model.add(Conv2D(filters= 32,kernel_size= (3,3), padding='same', kernel_initializer='he_normal',
                 input_shape=(rows, col, 1), activation='elu'))
...
check = ModelCheckpoint('/home/chime/PycharmProjects/Testthesis/Emotions.h4',monitor='val_loss', mode='min', save_best_only=True, verbose=1)
## Stop training when performance measure stops improving after 5 times i.e patience
stop = EarlyStopping(monitor='val_loss', mode='min', min_delta=0, patience=80, verbose=1, restore_best_weights=True)
...
learning_rate = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=80, verbose=1, min_delta=0.0001)
...
history = model.fit_generator(data_training_generator, steps_per_epoch=training_samples//size,
                              ...
                              validation_steps= test_samples//size)
